...
if ($a=="belum diterima") 
{
echo "<script>alert('Customer belum Mengkonfirmasi bahwa Dia sudah menerima barang.'); window.location = 'indexAdmin.php?page=laporan'</script>";
}
else{
mysqli_query($conn,"DELETE FROM chekout WHERE id_chekout='$kd'");
mysqli_query($conn,"DELETE FROM keranjang where id_pembeli='$id'");
header("location:indexAdmin.php?page=laporan");}
 ?>